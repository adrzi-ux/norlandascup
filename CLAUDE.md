...
- [x] **4 pages matériel publiées 2026-06-09** : IDs 30 (voilerie), 31 (electronique), 32 (securite), 33 (vetements-techniques) — contenu ~800 mots chacune, 4-6 H2, FAQ, liens internes croisés vers articles magazine. Noindex retiré sur les 4. Rank Math (title + description + focus_keyword) définis sur les 4.
- [x] **Encodage corrigé 2026-06-09** : IDs 28, 29, 30, 31, 32, 33 — caractères accentués reconstruits (é → ?)
- [x] **19 articles magazine — audit SEO + corrections factuelles 2026-06-09** : VHF canal corrigé (port-en-bessin : 9→18), marques citées (winches : Harken/Lewmar/Andersen/Barton/Antal, compas : Plastimo/Tacktick/KVH), Tour des Ports de la Manche 2026 ajouté (calendrier), Hydranet ajouté (voiles-avant + garde-robe), CROSS Jobourg VHF 79 (lire-grain), sas plaisance vs commercial (ouistreham-ecluse), GPS + SHOM (cherbourg-mouillages), tidal data (honfleur-avant-port). Noindex vérifié OK sur les 19 articles (aucun rank_math_robots défini).
- [x] **26 images remplacées Pexels 2026-06-09** : pipeline fetch-pexels.mjs → optimize-images.mjs (WebP q=88, ~1880px). Crédits dans `tools/credits.json`.
- [x] **CSS `.ph::after` supprimé 2026-06-09** : overlay caption data-ph retiré de theme.css (`.ph::after` + `.ph.dark::after`).
- [x] **Catégorie WP "Blog" créée (ID 3)** — pour futurs articles Wisewand (post_type=post).
